...
export function DataTable<TData, TValue>({
  ...
  const defaultSearchColumn = searchColumn || Object.keys(data[0] || {})[0] || "name";
                ...
                <TableRow key={headerGroup.id} className="border-b border-gray-100/80 bg-gradient-to-r from-gray-50/80 to-white/80">
                  {headerGroup.headers.map((header) => {
                    return (
                      <TableHead 
                        key={header.id} 
                        className="px-6 py-4 text-left font-semibold text-gray-700 bg-gradient-to-r from-gray-50/90 to-white/90 border-b-2 border-gray-100"
                      >
                ...
                {table.getRowModel().rows?.length ? (
                  table.getRowModel().rows.map((row, index) => (
                    <motion.tr
                      ...
                      className="border-b border-gray-50 hover:bg-gradient-to-r hover:from-orange-50/30 hover:to-amber-50/30 transition-all duration-300 group"
                      data-state={row.getIsSelected() && "selected"}
                    >
                      {row.getVisibleCells().map((cell) => (
                        <TableCell 
                          key={cell.id} 
                          className="px-6 py-4 text-gray-700 group-hover:text-gray-900 transition-colors duration-300"
                        >
                          {flexRender(
      ...
      >
        ...
        <div className="flex items-center space-x-2">
          <Button
            ...
            className="bg-white/50 border-gray-200/50 hover:bg-gradient-to-r hover:from-orange-50 hover:to-amber-50 disabled:opacity-50 transition-all duration-300"
          >
